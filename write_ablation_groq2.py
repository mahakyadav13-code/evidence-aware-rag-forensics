...
def generate_from_evidence_subset(evidence_subset, all_evidence_file):
    ...
    prompt = "Based on this evidence, write a brief investigator summary citing evidence IDs.\\n\\nEVIDENCE:\\n" + context + "\\n\\nWrite the summary now."
    return generate_with_retry(prompt)

def coverage(report_text, all_evidence):
    cited = set(re.findall(r"\\[([A-Z0-9\\-]+)\\]", report_text))
    valid = set(e["evidence_id"] for e in all_evidence)
...
with open("eval/ablation_study_v2.py", "w") as f:
    f.write(content)

print("ablation_study_v2.py updated with llama-3.1-8b-instant model")